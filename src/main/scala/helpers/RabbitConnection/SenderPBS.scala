...
object SenderPBS {
  ...
  def apply[A](queueName: QueueName, exchangeName: ExchangeName, routingKey: ShortString, message: Array[Byte]) = {
    val sender = new SenderPBS[A](queueName, exchangeName, routingKey, message)
    (sender, connect)
  }
}

class SenderPBS[A](val queueName: QueueName, val exchangeName: ExchangeName, val routingKey: ShortString, val message: Array[Byte]) {
